 ...
 * @author May
 * 
 *         广告项目二期数据处理 和1比较2加上了设备去重 和10分钟一次计算
 *
 */
public class SparkStreamingAdLogStats2OwnTest {

	public static void main(String[] args) throws InterruptedException {

		// 创建sparksession对象
		SparkSession sparkSession = SparkSession.builder().appName("StreamingRealTimeLogOnLineTest").getOrCreate();

		// 通过sparksession对象获取JavaSparkContext
		JavaSparkContext jsc = new JavaSparkContext(sparkSession.sparkContext());

		// 通过JavaSparkContext获取JavaStreamingContext
		JavaStreamingContext jssc = new JavaStreamingContext(jsc, Durations.seconds(300));
		...
		// 获取最基础的DStream
		JavaDStream<String> basicLogInfo = SparkStreamingAdLogStats2OwnTest.getBasicLogInfo(adRealTimeLogDStream);

		// 获取基础的数据过滤掉关键字段不为空的数据
		JavaDStream<String> filterLogInfo = SparkStreamingAdLogStats2OwnTest.getBasicLog(basicLogInfo);

		// 媒资管理平台获取广告曝光统计
		SparkStreamingAdLogStats2OwnTest.getOneHourExpose(sparkSession, filterLogInfo);
		// 获取广告投放统计
		SparkStreamingAdLogStats2OwnTest.getOneHourServing(sparkSession, filterLogInfo);
		// Yi+运营平台 和 广告投放平台 广告曝光统计
		SparkStreamingAdLogStats2OwnTest.getMasterOneHourExpose(sparkSession, filterLogInfo);
		// 运营平台设备统计
		SparkStreamingAdLogStats2OwnTest.devicesStats(sparkSession, filterLogInfo);
		// 运营平台媒体主统计
		SparkStreamingAdLogStats2OwnTest.mediaMasterStats(sparkSession, filterLogInfo);
		// 设备行为信息统计 输入的是最开始的数据 不是过滤后的
		SparkStreamingAdLogStats2OwnTest.devInfo(sparkSession, basicLogInfo);
