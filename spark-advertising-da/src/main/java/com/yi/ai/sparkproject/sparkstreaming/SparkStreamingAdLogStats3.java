 ...
 * @author May
 * 
 *         广告项目二期数据处理 和1比较2加上了设备去重 和10分钟一次计算 3加上了判断是否是json 这个是上线版本
 * 
 * 
 *         注意！！！注意！！！ 版本迭代时候checkpoint 设置在不同的目录里
 *         
 *         （原因持久化时候可能将上个版本代码也读过来了）
 *
 */
public class SparkStreamingAdLogStats3 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// jssc.checkpoint("hdfs://172.17.133.117:8020//adSysLogCheckPoint");
		...
		// 获取最基础的DStream
		JavaDStream<String> basicLogInfo = SparkStreamingAdLogStats3.getBasicLogInfo(adRealTimeLogDStream);

		// 获取基础的数据过滤掉关键字段不为空的数据
		JavaDStream<String> filterLogInfo = SparkStreamingAdLogStats3.getBasicLog(basicLogInfo);

		// 媒资管理平台获取广告曝光统计
		SparkStreamingAdLogStats3.getOneHourExpose(sparkSession, filterLogInfo);
		// 获取广告投放统计
		SparkStreamingAdLogStats3.getOneHourServing(sparkSession, filterLogInfo);
		// Yi+运营平台 和 广告投放平台 广告曝光统计
		SparkStreamingAdLogStats3.getMasterOneHourExpose(sparkSession, filterLogInfo);
		// 运营平台设备统计
		SparkStreamingAdLogStats3.devicesStats(sparkSession, filterLogInfo);
		// 运营平台媒体主统计
		SparkStreamingAdLogStats3.mediaMasterStats(sparkSession, filterLogInfo);
		// 设备行为信息统计 输入的是最开始的数据 不是过滤后的
		SparkStreamingAdLogStats3.devInfo(sparkSession, basicLogInfo);
	...
	private static void getOneHourExpose(SparkSession sparkSession, JavaDStream<String> filterLogInfoCache) {
		...
		reduceByKey.foreachRDD(new VoidFunction<JavaPairRDD<String, Long>>() {
			...
			public void call(JavaPairRDD<String, Long> t) throws Exception {
				...
				String url = "jdbc:mysql://172.17.133.119:3306/adSys?useUnicode=true&characterEncoding=utf-8";
