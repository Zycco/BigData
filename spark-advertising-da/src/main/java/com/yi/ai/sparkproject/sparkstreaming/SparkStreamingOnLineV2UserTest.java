...
public class SparkStreamingOnLineV2UserTest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 创建sparksession对象
		SparkSession sparkSession = SparkSession.builder().appName("UserTestStreamingRealTimeLogOnLine").getOrCreate();
		...
		topics.add("UserTestTopic");

		// 创建最基础的输入DStream
		JavaPairInputDStream<String, String> adRealTimeLogDStream = KafkaUtils.createDirectStream(jssc, String.class,
				String.class, StringDecoder.class, StringDecoder.class, kafkaParams, topics);

		JavaDStream<String> basicLogInfo = SparkStreamingOnLineV2UserTest.getBasicLogInfo(adRealTimeLogDStream);
		// 广告曝光统计小时为粒度
		SparkStreamingOnLineV2UserTest.getOneHourExpose(sparkSession, basicLogInfo);
		// 广告投放统计
		SparkStreamingOnLineV2UserTest.getOneHourServing(sparkSession, basicLogInfo);
	...
	private static JavaDStream<String> getBasicLogInfo(JavaPairInputDStream<String, String> adRealTimeLogDStream) {
		...
		JavaDStream<String> filter = map.filter(new Function<String, Boolean>() {
			...
			public Boolean call(String v1) throws Exception {
				...
				return (adlogInfo[0] != null && adlogInfo[0].length() != 0 && adlogInfo[1] != null
						&& adlogInfo[1].length() != 0 && adlogInfo[2] != null && adlogInfo[2].length() != 0
						&& adlogInfo[4] != null && adlogInfo[4].length() != 0 && adlogInfo[5] != null
						&& adlogInfo[5].length() != 0 && adlogInfo[6] != null && adlogInfo[6].length() != 0
						&& adlogInfo[7] != null && adlogInfo[7].length() != 0 && adlogInfo[9] != null
						&& adlogInfo[9].length() != 0 && adlogInfo[11] != null && adlogInfo[11].length() != 0
						&& adlogInfo[12] != null && adlogInfo[12].length() != 0 && adlogInfo[14] != null
						&& adlogInfo[14].length() != 0);
			}
	...
	private static void getOneHourExpose(SparkSession sparkSession, JavaDStream<String> map) {
		...
		transform.foreachRDD(new VoidFunction<JavaRDD<Row>>() {
			...
			public void call(JavaRDD<Row> t) throws Exception {
				...
				String url = "jdbc:mysql://172.17.133.113:3306/adsys_usertest?useUnicode=true&characterEncoding=utf-8";
