 ...
 *
 */
public class SparkStreamingAdLogStats {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 获取最基础的DStream
		JavaDStream<String> basicLogInfo = SparkStreamingAdLogStats.getBasicLogInfo(adRealTimeLogDStream);

		// 获取基础的数据过滤掉关键字段不为空的数据
		JavaDStream<String> filterLogInfo = SparkStreamingAdLogStats.getBasicLog(basicLogInfo);

		// 媒资管理平台获取广告曝光统计
		SparkStreamingAdLogStats.getOneHourExpose(sparkSession, filterLogInfo);
		// 获取广告投放统计
		SparkStreamingAdLogStats.getOneHourServing(sparkSession, filterLogInfo);
		// Yi+运营平台 和 广告投放平台 广告曝光统计
		SparkStreamingAdLogStats.getMasterOneHourExpose(sparkSession, filterLogInfo);
		// 运营平台设备统计
		SparkStreamingAdLogStats.devicesStats(sparkSession, filterLogInfo);
		// 运营平台媒体主统计
		SparkStreamingAdLogStats.mediaMasterStats(sparkSession, filterLogInfo);
		// 设备行为信息统计 输入的是最开始的数据 不是过滤后的
		SparkStreamingAdLogStats.devInfo(sparkSession, basicLogInfo);
